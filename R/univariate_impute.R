...
uni.impute.dvinemed <- function(x,uscale=FALSE,print_pair=FALSE,alpha=0.5){
  ...
  if(!uscale){
    ...
    for(i in seq(1,ncol(x_train),1)){
      fit_dist = fitDist(unlist(x_train[,i]), type="realline")
    ...
    if(print_pair) pairs_copula_data(x_train,col='red')
  ...
  fit = vinereg(as.formula(fitstr),data=u_train, uscale = TRUE)
  if(is.null(ncol(x_pred))){
    u_pred = data.frame(u_pred)
    names(u_pred) = colnames(x)[2]
  }
  
  u_impute = predict(fit, newdata = u_pred, alpha = alpha)
  if(!uscale)
...
uni.impute.dvinemean <- function(x,uscale=FALSE,print_pair=FALSE){
  ...
  u_impute = predict(fit, newdata = u_pred, alpha = NA)
  if(!uscale)
    x_impute = q_fit(unlist(u_impute),fit_list[[1]])
  else 
    x_impute = unlist(u_impute)
...
uni.impute.lm <- function(x,print_pair=FALSE){
  missing_vec = seq(length(x[,1]))[is.na(x[,1])]
  if(print_pair) ggpairs(x[-missing_vec])
  fit = lm(as.formula(paste(colnames(x)[1], "~.")),data=x[-missing_vec,])
  x_impute = predict(fit,newdata = x[missing_vec,])
  x[missing_vec,1] = x_impute
  return(x)
}
...
uni.impute.knn <- function(x,print_pair=FALSE){
  missing_vec = seq(length(x[,1]))[is.na(x[,1])]
  if(print_pair) ggpairs(x[-missing_vec])
  return(data.frame(impute.knn(as.matrix(x))$data))
}
...
uni.impute.rq <- function(x,print_pair=FALSE,tau=0.5){
  missing_vec = seq(length(x[,1]))[is.na(x[,1])]
  if(print_pair) ggpairs(x[-missing_vec])
  fit = rq(as.formula(paste(colnames(x)[1], "~.")),data=x[-missing_vec,],tau=tau)
  x_impute = predict(fit,newdata = x[missing_vec,])
  x[missing_vec,1] = x_impute
  return(x)
}
...
uni.impute.rlm <- function(x,print_pair=FALSE){
  missing_vec = seq(length(x[,1]))[is.na(x[,1])]
  if(print_pair) ggpairs(x[-missing_vec])
  fit = rlm(as.formula(paste(colnames(x)[1], "~.")),data=x[-missing_vec,])
  x_impute = predict(fit,newdata = x[missing_vec,])
  x[missing_vec,1] = x_impute
  return(x)
}
...
uni.impute.lqr <- function(x,print_pair=FALSE,p=0.5){
  missing_vec = seq(length(x[,1]))[is.na(x[,1])]
  if(print_pair) ggpairs(x[-missing_vec])
  fit_lqr = lqr(as.formula(paste(colnames(x)[1], "~.")),data=x[-missing_vec,],p=p)
